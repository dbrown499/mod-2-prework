...
searchButton[0].addEventListener('click', () => {
        let inputValue = (inputBox[0].value).toLowerCase().split(' ').join('');
...
function getWeather (input){
    ...
    windSpeed[0].innerHTML = Math.round(input.wind.speed) +" km/h";

    const humidity = document.getElementsByClassName("humidity");
    humidity[0].innerHTML = Math.round(input.main.humidity) +"%";

    const name = document.getElementsByClassName("city");
    name[0].innerHTML = input.name;

    const degrees = document.getElementsByClassName("temp");
    degrees[0].innerHTML = Math.round(input.wind.deg) + "°c";
